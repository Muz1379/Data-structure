...
int contexec( char * str, struct BINTREE * hashT[] )
{
	...
	strEnd = kmp( str, youqinglianjie, 0 );							//友情链接---12399
	strP = kmp( str, fankui, 0 );									//反馈---169
	...
	for(; strP < strEnd; strP++)
	{
		...
		*((memberT->qqNum) + j) = '\0';
		...
		if(*(memberT->qqNum) == '\0')//读入完最后一个数据项后，由于strP距离结束点仍有一段距离，所以会创建一个空项。该条件用于判断是否为空，若空，则返回，不插入，表明已完成。
		{
			return OK;
		}
