...
int contexec( char * str, struct BINTREE * hashT[] )
{
	...
	struct MEMBERNODE * memberT;					   					   //过渡存放成员内存指针
	...
	do
	{
		...
		j++;
		strP++;
	} while(*(str + strP) != (char)0x29);		   	   					   //TODO:此处字符转换问题
										   			   					   //群名片读取结束

	for(i = 0; i !=-1;)
	{
		i = kmp( str, "最后发言", strP );
		if((i > 0)&&(i<strEnd))						   					   //找到“最后发言”，字符串游标后移
		{
			strP = i + sizeof( "最后发言" );
		}
		else							   			   					   //找不到“最后发言”，跳出循环，结束查找。
			break;
	}
